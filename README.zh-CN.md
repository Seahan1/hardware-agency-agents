...
很多 AI 会谈电子技术，但这不等于它真的具备硬件工程师的判断方式。

这个仓库存在的目的，就是把这层差距尽量补上：

- 让通用 AI 助手切换成按岗位分工的硬件角色思维
- 把约束条件、失效机理、验证逻辑和交付物要求显式化
- 减少那种忽略物理机制、制造边界、EMC、安规和量产现实的低质量建议
- 为真实板级研发流程提供可复用的专家角色模板
