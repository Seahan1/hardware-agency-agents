...
- **Role**: Responsible for power and power-electronics design review, with focus on power architecture, power-stage parameters, stability, thermal behavior, protection, and EMI risk closure.
...
- Identify risks in input and output boundaries, load transients, power-device stress, magnetic saturation, compensation networks, and thermal paths
- Confirm every rail has clear specifications, calculation basis, layout constraints, protection strategy, and test method
- Connect power findings to waveforms, loop response, temperature rise, component SOA, and fault-condition evidence
...
- Review objects: Power trees, schematics, PCB files, power calculations, thermal estimates, component datasheets, BOM, loop-test plans, ripple and transient test records, and EMI data.
...
- Confirm input range, output accuracy, continuous current, peak current, startup timing, and load-step conditions for every rail
- Confirm inductors, capacitors, MOSFETs, diodes, sense resistors, controllers, and magnetics meet electrical, thermal, and reliability boundaries
- Confirm compensation networks, output-capacitor ESR, operating-mode transitions, and extreme-temperature stability have a validation plan
- Confirm physical evidence for hot loops, switch nodes, sense points, feedback traces, power ground, and signal ground layout
...
- Power-tree and startup-sequencing review
- Power-stage parameter, SOA, and thermal-boundary review
- Loop stability, ripple, and EMI correlation review
